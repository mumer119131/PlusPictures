...
const getPhoto = async (id) => {
  if (id.endsWith("us")) {
    const image = await axios(
      `https://api.unsplash.com/photos/${id.slice(0, -2)}?client_id=${
    ...
    const data = {
      ...
      data: {
        ...
        type: "image",
        alt: image.data.alt_description,
        user_detail: image.data.user,
        id: image.data.id + "us",
        url: image.data.links.html,
        user_url: image.data.user.links.html,
      },
    };
    return data;
    ...
    const image = await axios(
      `https://pixabay.com/api/videos?key=${
        import.meta.env.VITE_PIXBAY_KEY
      }&id=${id.slice(0, -2)}`
    );
    ...
    for (let video of Object.keys(videos)) {
      video_sizes = [
        ...video_sizes,
        { quality: video, link: videos[video].url },
      ];
    }
    const data = {
      ...image.data.hits[0],
    ...
    try {
      image = await axios(
        `https://api.pexels.com/v1/photos/${id.slice(0, -2)}`,
        { headers: { Authorization: import.meta.env.VITE_PEXELS_KEY } }
      );
    } catch {
      image = await axios(
        `https://api.pexels.com/videos/videos/${id.slice(0, -2)}`,
        { headers: { Authorization: import.meta.env.VITE_PEXELS_KEY } }
      );
    }
    var data = [];
    ...
    if (image.data.video_files) {
      ...
      for (let video of image.data.video_files) {
        ...
        video_sizes = [
          ...video_sizes,
          { quality: video.quality, link: video.link },
        ];
      }
      ...
      const regex = /\/(\w+(?:-\w+)*)-\d+\./;
      const match = image.data.image.match(regex);
      ...
      data = {
        ...
        data: {
          ...
          url: image.data.url,
          alt: image.data.alt,
          user_url: image.data.photographer_url,
        },
      };
    }
    return data;
...
const getUnSplashImages = async (query = randomWords(), page) => {
  ...
  if (query) {
    url = `https://api.unsplash.com/search/photos?query=${query}&client_id=${
      import.meta.env.VITE_UNSPLASH_ACCESS_KEY
    }&page=${page}`;
  }
  try {
